...
impl VectorBackend {
    fn from_env() -> Self {
        match std::env::var("CRABDB_SQLITE_VECTOR_BACKEND")
            ...
            .as_str()
        {
            ...
            other => {
                panic!("unsupported CRABDB_SQLITE_VECTOR_BACKEND={other}; use sqlite_vec0 or exact")
            }
        }
    }
...
fn main() {
    ...
    let rows = env_usize("CRABDB_SQLITE_VECTOR_ROWS").unwrap_or(DEFAULT_ROWS);
    let dims = env_usize("CRABDB_SQLITE_VECTOR_DIMS").unwrap_or(DEFAULT_DIMS);
    let queries = env_usize("CRABDB_SQLITE_VECTOR_QUERIES").unwrap_or(DEFAULT_QUERIES);
    let top_k = env_usize("CRABDB_SQLITE_VECTOR_TOP_K").unwrap_or(DEFAULT_TOP_K);
    let keep_db = std::env::var("CRABDB_SQLITE_VECTOR_KEEP_DB")
        .ok()
        .as_deref()
        == Some("1");
    let path = db_path();
...
fn db_path() -> PathBuf {
    if let Ok(path) = std::env::var("CRABDB_SQLITE_VECTOR_DB") {
    ...
    std::env::temp_dir().join(format!(
        "crabdb-sqlite-vector-memory-{}-{nanos}.db",
        std::process::id()
    ))
}
