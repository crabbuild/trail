...
pub(super) enum AgentSubcommand {
    ...
    /// Show changes grouped by turn or operation without chasing ids.
...
#[derive(Args)]
#[command(
    after_help = "Run `crabdb agent` with no subcommand to show the agent inbox: grouped tasks and one next action."
)]
