...
impl CrabDb {
    pub fn rewind_agent(
        ...
        if record_current {
            ...
            {
                let report = self.record_agent_workdir(
                    agent,
                    Some(format!("Record current agent `{agent}` before rewind")),
                )?;
        ...
        let mut report = {
            ...
            let change_id = self.allocate_change_id(&actor.id, "agent_rewind")?;
            let message = match &preserved_branch {
                Some(branch) => format!(
                    "Rewind agent `{agent}` to `{target}`; preserved previous head as `{branch}`"
                ),
                None => format!("Rewind agent `{agent}` to `{target}`"),
            };
            let operation = Operation {
                version: OP_OBJECT_VERSION,
                change_id: change_id.clone(),
                kind: OperationKind::AgentRewind,
            ...
            self.conn.execute(
                "UPDATE agent_branches SET head_change = ?1, head_root = ?2, status = 'active', updated_at = ?3 WHERE agent_id = ?4",
                params![change_id.0, target_ref.root_id.0, now_ts(), branch.agent_id],
            )?;
        ...
        if sync_workdir && report.workdir.is_some() {
            let sync = self.sync_agent_workdir(agent, true)?;
    ...
    fn preserve_rewind_head_branch(&self, agent: &str, head: &RefRecord) -> Result<String> {
        let short_change = short_rewind_change_id(&head.change_id);
        for suffix in 0..1000u16 {
            let branch = if suffix == 0 {
                format!("rewind/{agent}/{short_change}")
            } else {
                format!("rewind/{agent}/{short_change}-{suffix}")
            };
        ...
        Err(Error::InvalidInput(format!(
            "could not allocate rewind preservation branch for `{agent}`"
        )))
    }
}
