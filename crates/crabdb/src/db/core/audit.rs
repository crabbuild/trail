...
impl CrabDb {
    pub(crate) fn record_external_mutation_audit(
        ...
        self.conn.execute(
            "INSERT INTO external_mutation_audit \
             (audit_id, surface, command, target_ref, lane_id, status, status_code, change_id, summary_json, created_at) \
    ...
    pub fn list_external_mutation_audit(
        ...
        let mut stmt = self.conn.prepare(
            "SELECT audit_id, surface, command, target_ref, lane_id, status, status_code, change_id, summary_json, created_at \
...
fn external_mutation_audit_row(
    ...
    Ok(ExternalMutationAuditRecord {
        ...
        change_id: row.get::<_, Option<String>>(7)?.map(ChangeId),
        summary,
