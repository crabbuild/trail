...
impl HttpMutationAudit {
    ...
    pub(super) fn record_idempotency_replay(self, db: &mut CrabDb, response: &HttpResponse) {
    ...
    fn record_with_context(self, db: &mut CrabDb, response: &HttpResponse, context: Option<Value>) {
...
fn http_actor(request: &HttpRequest) -> String {
    ...
    if request.headers.contains_key("x-crabdb-token") {
        return "http:x-crabdb-token".to_string();
    }
