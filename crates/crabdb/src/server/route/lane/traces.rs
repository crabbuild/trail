...
pub(super) fn handle_trace_routes(
    ...
    if request.method == "GET" && path == "/v1/lane/spans" {
    ...
    if request.method == "GET" && path == "/v1/lane/spans/summary" {
    ...
    if parts.len() == 4
        ...
        && request.method == "GET"
    {
        let span = db.show_lane_trace_span(parts[3])?;
        ...
        && request.method == "POST"
    {
        ...
        let report = db.end_lane_trace_span(parts[3], &body.status, body.result)?;
        return Ok(Some(json_response(200, "OK", &report)?));
    }
