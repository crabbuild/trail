...
trail agent continue [latest|<TASK_OR_LANE_OR_SESSION>] \
  ...
  [--workdir-mode auto|native-cow|portable-copy|fuse-cow|nfs-cow|dokan-cow] \
  [--allow-project-integrations] [-- <COMMAND>...]
```
...
resolvers currently require an offline, credential-free plan. Cargo resolution
declares `CARGO_HOME` explicitly and may read the invoking host's existing
Cargo registry/Git cache; it never contacts the network during resolution.
Use `--path .` to select only the repository-root component when a checkout
also contains independent nested projects. Repository and
