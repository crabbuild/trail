...
Use `trail config` to inspect and edit typed workspace config values.
...
```sh
trail config get lane.default_materialize
```
...
```sh
trail config set lane.required_test_suites "unit,integration"
```
...
```sh
trail config set guardrails.policy "block:path:production;approval:keyword:deploy"
```
