...
export class CrabDbDaemonClient {
  constructor(private readonly workspaceRoot: string) {}

  async discover(): Promise<DaemonEndpoint | undefined> {
    const endpointPath = path.join(this.workspaceRoot, ".crabdb", "daemon.json");
    try {
  ...
  private async requestJson<T>(
    ...
    return new Promise<T>((resolve, reject) => {
        ...
        (response) => {
          ...
          response.on("end", () => {
            if (!response.statusCode || response.statusCode >= 400) {
              reject(new Error(`CrabDB daemon ${method} ${route} failed: ${response.statusCode} ${data}`));
              return;
            }
            try {
              resolve(JSON.parse(data) as T);
            } catch (error) {
              reject(new Error(`CrabDB daemon returned invalid JSON for ${route}: ${String(error)}`));
            }
          });
        }
      );
      ...
      request.on("timeout", () => {
        request.destroy(new Error(`CrabDB daemon ${method} ${route} timed out`));
      });
      if (payload) {
  ...
  private async readToken(): Promise<string | undefined> {
    try {
      return (await fs.readFile(path.join(this.workspaceRoot, ".crabdb", "daemon.token"), "utf8")).trim();
    } catch {
      return undefined;
    }
  }
