...
import { TrailCli } from "./TrailCli";
import { TrailDaemonClient, type DaemonEndpoint } from "./TrailDaemonClient";
...
export class TaskRepository {
  private readonly cli: TrailCli;
  private readonly daemon: TrailDaemonClient;
  ...
  constructor(
    ...
    this.cli = new TrailCli(workspaceRoot, output);
    this.daemon = new TrailDaemonClient(workspaceRoot);
  }
  ...
  async initWorkspace(): Promise<void> {
    ...
    if (result.code !== 0) {
      throw new Error(result.stderr.trim() || `Trail init failed with code ${result.code}`);
    }
  }

  isWorkspaceInitialized(): boolean {
    return fs.existsSync(path.join(this.workspaceRoot, ".trail"));
  }
...
function normalizeTaskView(report: unknown, selector: string): TaskView {
  ...
  return {
    task: {
      ...task,
      sessionId: task.sessionId || stringField(acpSession, "trail_session_id"),
