...
import { CrabDbDaemonClient } from "../crabdb/CrabDbDaemonClient";

test("CrabDbDaemonClient discovers endpoint and sends bearer token", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "crabdb-daemon-client-"));
  const crabDir = path.join(tempDir, ".crabdb");
  ...
  try {
    ...
    const client = new CrabDbDaemonClient(tempDir);
...
test("CrabDbDaemonClient posts JSON bodies", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "crabdb-daemon-client-"));
  const crabDir = path.join(tempDir, ".crabdb");
  fs.mkdirSync(crabDir);
...
test("CrabDbDaemonClient sends DELETE requests", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "crabdb-daemon-client-"));
  const crabDir = path.join(tempDir, ".crabdb");
  fs.mkdirSync(crabDir);
