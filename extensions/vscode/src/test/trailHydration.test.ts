...
import type { TaskView } from "../trail/TaskRepository";
...
test("hydrates persisted Trail transcript turns into render nodes", () => {
  ...
  assert.equal(nodes.every((node) => node.source === "trail"), true);
});

test("hydrates persisted Trail message content blocks when body is absent", () => {
...
test("hydrates persisted Trail message text fields when body is absent", () => {
...
test("hydrates persisted Trail string content when body is absent", () => {
  const textView: TaskView = {
    ...view,
    turns: [
      {
...
test("falls back to persisted Trail message row aliases when canonical content is empty", () => {
  const textView: TaskView = {
    ...view,
    turns: [
      {
...
test("hydrates persisted Trail mixed content arrays when body is absent", () => {
  const contentView: TaskView = {
    ...view,
    turns: [
      {
...
test("hydrates persisted Trail text content blocks with aliased text fields", () => {
  const contentView: TaskView = {
    ...view,
    turns: [
      {
...
test("hydrates persisted Trail message rows wrapped in nested message objects", () => {
  const contentView: TaskView = {
    ...view,
    turns: [
      {
...
test("hydrates persisted tool events into rich tool and terminal nodes", () => {
  ...
  assert.equal(tool?.source, "trail");
...
test("hydrates duplicate ACP message ids by consuming message-added events in order", () => {
  ...
  assert.deepEqual(
    ...
    [
      "trail-message:turn-duplicate-message-id:msg-user",
      "trail-message:turn-duplicate-message-id:msg-shared:1",
      "trail-message:turn-duplicate-message-id:msg-shared:2"
    ]
  );
});
...
test("replaces the matching duplicated-message-id continuation from Trail hydration", () => {
  ...
  assert.deepEqual(
    ...
    [
      "trail:user:msg-user:Run both checks",
      "trail:assistant:msg-shared:First check is next.",
      "trail:tool:first-check",
      "trail:assistant:msg-shared:Second check is next.",
      "trail:checkpoint:"
    ]
  );
  assert.equal(
...
test("keeps split live duplicated-message-id segments when Trail hydration is cumulative", () => {
  const current: RenderNode[] = [
    {
      id: "message:assistant:msg-shared:1",
      kind: "message",
  ...
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-cumulative-message-id:msg-shared",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-cumulative-message-id",
  ...
  assert.deepEqual(
    ...
    [
      ...
      "trail:tool:read-context",
      "acp-live:After the tool."
    ]
  );
  assert.equal(
    merged.some((node) => node.kind === "message" && node.source === "trail" && node.text === "Before the tool. After the tool."),
    false
  );
...
test("hydrates duplicate tool ids across turns with unique render ids", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      ["tool:shared-tool:turn-tool-b:trail", "turn-tool-b", "Run shared tool B"]
    ]
  );
  ...
  assert.deepEqual(
    ...
    [
      ...
      ["terminal:shared-tool:shared-terminal:turn-tool-b:trail", "turn-tool-b", "B"]
    ]
  );
});
...
test("preserves live transcript order when replacing a completed stream with hydrated nodes", () => {
  ...
  assert.deepEqual(
    ...
    "this fixture should reproduce the lossy Trail fallback order"
  );
...
test("keeps missing earlier hydrated messages before matched live messages", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-partial-anchor:user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
    ...
    {
      id: "trail-message:turn-partial-anchor:assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-partial-anchor",
  ...
  assert.deepEqual(
    ...
    [
      "trail:user:Explain the extension",
      "trail:assistant:It renders ACP updates."
    ]
  );
...
test("reconciles completed live messages with hydrated messages when the live turn id is missing", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-missing-live-turn:msg-user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
    ...
    {
      id: "trail-message:turn-missing-live-turn:msg-assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-missing-live-turn",
  ...
  assert.deepEqual(
    merged.map((node) => (node.kind === "message" ? `${node.source}:${node.role}:${node.text}` : `${node.source}:${node.kind}`)),
    [
      "trail:user:Explain rendering",
      "trail:assistant:Rendering is stable."
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2]);
});
...
test("reconciles missing-turn live messages when hydrated text extends live text", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-missing-live-turn-prefix:msg-user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
    ...
    {
      id: "trail-message:turn-missing-live-turn-prefix:msg-assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-missing-live-turn-prefix",
  ...
  assert.deepEqual(
    merged.map((node) => (node.kind === "message" ? `${node.source}:${node.role}:${node.text}` : `${node.source}:${node.kind}`)),
    [
      "trail:user:Explain final rendering",
      "trail:assistant:Rendering is stable after completion."
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2]);
});
...
test("reconciles completed live turn with hydrated turn when the user prompt matches", () => {
  const hydrated: RenderNode[] = [
    ...
    {
      id: "trail-message:turn-trail:msg-assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-trail",
  ...
  assert.deepEqual(
    ...
    [
      "trail:turn-trail:user:What I have in current repo",
      "trail:turn-trail:list-files",
      "trail:turn-trail:assistant:Here is the summary."
    ]
  );
...
test("drops checkpoint-pending live completion when aliased hydrated turn has a checkpoint", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-trail-checkpoint:msg-user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-trail-checkpoint",
      provider: "provider",
      source: "trail",
      status: "completed",
      role: "user",
    ...
    {
      id: "trail-message:turn-trail-checkpoint:msg-assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-trail-checkpoint",
    ...
    {
      id: "trail-checkpoint:turn-trail-checkpoint",
      kind: "checkpoint",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-trail-checkpoint",
  ...
  assert.deepEqual(
    ...
    [
      "trail:turn-trail-checkpoint:user:what I have in current repo and how many lines of code ?",
      "trail:turn-trail-checkpoint:tool:completed",
      "trail:turn-trail-checkpoint:assistant:So the repo has about 5,357 lines of code.",
      "trail:turn-trail-checkpoint:checkpoint:completed"
    ]
  );
...
test("reconciles hydrated transcript while keeping only checkpoint-pending completion", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-trail-pending:msg-user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-trail-pending",
      provider: "provider",
      source: "trail",
      status: "completed",
      role: "user",
    ...
    {
      id: "trail-message:turn-trail-pending:msg-assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-trail-pending",
  ...
  assert.deepEqual(
    ...
    [
      "trail:turn-trail-pending:user:what I have in current repo and how many lines of code ?",
      "trail:turn-trail-pending:tool:completed",
      "trail:turn-trail-pending:assistant:So the repo has about 5,357 lines of code.",
      "acp-live:turn-trail-pending:completion:pending"
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2, 3, 4]);
  ...
  const settled = mergeHydratedNodes(
    [
      ...hydrated,
      {
        id: "trail-checkpoint:turn-trail-pending",
        kind: "checkpoint",
        taskId: "task-1",
        lane: "lane-1",
        turnId: "turn-trail-pending",
  ...
  assert.deepEqual(
    ...
    [
      "trail:turn-trail-pending:message:completed",
      "trail:turn-trail-pending:tool:completed",
      "trail:turn-trail-pending:message:completed",
      "trail:turn-trail-pending:checkpoint:completed"
    ]
  );
});

test("reconciles newly claimed Trail task with live new-task prompt scope", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-trail-new-task:msg-user",
      kind: "message",
      taskId: "task-claimed",
      lane: "lane-claimed",
      turnId: "turn-trail-new-task",
      provider: "provider",
      source: "trail",
      status: "completed",
      role: "user",
    ...
    {
      id: "trail-message:turn-trail-new-task:msg-assistant",
      kind: "message",
      taskId: "task-claimed",
      lane: "lane-claimed",
      turnId: "turn-trail-new-task",
  ...
  assert.deepEqual(
    ...
    [
      "trail:task-claimed:lane-claimed:turn-trail-new-task:user:what I have in current repo and how many lines of code ?",
      "trail:task-claimed:lane-claimed:turn-trail-new-task:tool:completed",
      "trail:task-claimed:lane-claimed:turn-trail-new-task:assistant:So the repo has about 5,357 lines of code.",
      "acp-live:task-claimed:lane-claimed:turn-trail-new-task:completion:pending"
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2, 3, 4]);
});
...
test("does not reconcile a checkpoint-pending repeat prompt with an older hydrated turn", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-old-repeat:msg-user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
    ...
    {
      id: "trail-message:turn-old-repeat:msg-assistant",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
      turnId: "turn-old-repeat",
  ...
  assert.deepEqual(
    ...
    [
      "trail:turn-old-repeat:user:What I have in current repo",
      "trail:turn-old-repeat:assistant:Old summary.",
...
test("reconciles completed live tools with hydrated tools when the live turn id is missing", () => {
  const hydrated: RenderNode[] = [
    {
      id: "trail-message:turn-missing-live-tool:msg-user",
      kind: "message",
      taskId: "task-1",
      lane: "lane-1",
  ...
  assert.deepEqual(
    ...
    [
      "trail:user:Run status",
      "trail:tool:Run status"
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2]);
});
...
test("keeps distinct same-tool diff children when hydrating one diff", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "trail:src/b.ts:new b with hydrated details"
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2]);
});
...
test("keeps distinct same-request approval children when hydrating one approval", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "trail:tool-b:Approve second command with persisted details"
    ]
  );
  assert.deepEqual(merged.map((node) => node.timelineOrder), [1, 2]);
});
...
test("keeps completed live assistant text until Trail hydration contains an equivalent message", () => {
  const current: RenderNode[] = [
    {
...
test("keeps richer completed live assistant text when hydrated message is truncated", () => {
  const hydrated: RenderNode = {
    id: "trail-message:turn-truncated:msg-1",
    kind: "message",
    taskId: "task-1",
    lane: "lane-1",
...
test("keeps completed live assistant text when hydrated message body diverges", () => {
  const hydrated: RenderNode = {
    id: "trail-message:turn-divergent:msg-1",
    kind: "message",
    taskId: "task-1",
    lane: "lane-1",
...
test("keeps richer completed live assistant content when hydrated text is equivalent", () => {
  const hydrated: RenderNode = {
    id: "trail-message:turn-rich-content:msg-1",
    kind: "message",
    taskId: "task-1",
    lane: "lane-1",
...
test("keeps richer completed live assistant content when hydrated text extends live text", () => {
  const hydrated: RenderNode = {
    id: "trail-message:turn-rich-content-prefix:msg-1",
    kind: "message",
    taskId: "task-1",
    lane: "lane-1",
...
test("marks open tool events completed when hydrating a completed Trail turn", () => {
  const eventView: TaskView = {
    ...view,
    turns: [
      {
...
test("keeps in-progress live tools when older hydrated turns reuse provider ids", () => {
  ...
  assert.deepEqual(
    ...
    [
      "trail:Old read:tool:shared-provider-id",
...
test("drops checkpoint-pending completion placeholders after matching Trail checkpoint hydration", () => {
...
test("keeps checkpoint-pending completion placeholders until the matching Trail checkpoint exists", () => {
...
test("keeps failed live completion events after Trail hydration", () => {
  const liveCompletion: RenderNode = {
