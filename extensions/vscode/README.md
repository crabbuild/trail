# CrabDB Agents for VS Code

CrabDB Agents is a VS Code extension that provides a unified Agent Client
Protocol chat and review workflow backed by CrabDB.

The extension treats ACP as the live agent protocol and CrabDB as the durable
...
run **Run CrabDB Agents Extension** from the Run and Debug view.
...
- Contextual welcome states for no open folder, an uninitialized CrabDB
  workspace, empty task/review lists, and an empty merge queue, including a
  `CrabDB: Initialize Workspace` command.
- Task tree coordination badges derived from CrabDB readiness, including
...
- Command palette actions for workspace initialization, new task, open chat,
  ...
  points.
- Provider picking for new chats, with built-in CrabDB-backed Claude Code and
  configurable custom ACP commands.
- ACP JSON-RPC stdio client for `crabdb agent acp`.
...
- CrabDB-backed chats preflight workspace initialization before launching the
  ACP relay, offering the `CrabDB: Initialize Workspace` command instead of
...
- ACP session metadata updates surface provider-suggested titles in the chat
  header and review drawer without mutating the durable CrabDB task title.
- Reopen-aware ACP lifecycle handling: existing CrabDB ACP session IDs use
  `session/resume` or `session/load` only when the provider advertises support;
  otherwise the extension starts a checkpoint-based follow-up through CrabDB.
- Explicit provider switching inside the composer. Switching providers stops
  the active ACP process and starts the next prompt as a CrabDB checkpoint
...
- Prompt completion footers that render ACP stop reasons (`end_turn`,
  ...
  from durable CrabDB checkpoints.
...
- CrabDB daemon discovery with daemon-first reads/mutations for stable HTTP
...
- Current selection, current file, diagnostics, latest terminal output,
  changed-file list, and CrabDB history attachments sent as ACP content blocks
...
- Rehydration of persisted CrabDB transcript turns into the chat timeline.
- Review drawer with task summary, readiness, tests/evals, diff links,
  transcript jump links, CrabDB coordination signals, lane test/eval execution,
...
- Conflict detail drawer for CrabDB conflict sets reported by readiness/review,
...
- Daemon-enriched review/readiness data is merged into the review drawer when
  available, while the persisted transcript still reloads from CrabDB.
- Queue Tree View backed by CrabDB merge-queue entries, grouped by status with
...
- Redaction for sensitive keys, bearer tokens, CLI flags, raw ACP details, and
  CrabDB command output before they are shown in the webview or output channel.
...
- Extension-host smoke coverage for activation, commands, configuration, and a
  disposable `crabdb agent acp` ChatPanel run where a stub ACP provider modifies
  a materialized lane workdir and CrabDB records the transcript/checkpoint.
- Extension-host smoke coverage for `session/request_permission`: a stub ACP
  ...
  a materialized lane, and CrabDB records the approved transcript/checkpoint.
- CLI compatibility fallback for `Open diff`: when an older CrabDB CLI lacks
...
- Full daemon-backed agent task HTTP routes when CrabDB exposes them.
- Real-provider smoke tests through `crabdb agent acp` once credentials and
  provider CLIs are available in CI.

## Architecture

- The extension host launches `crabdb agent acp --provider <provider>` and
  speaks ACP JSON-RPC over stdio.
- The extension host reads persisted task state through CrabDB CLI/daemon
  helpers.
- The webview renders ACP content through protocol-shaped renderer registries.
- Durable transcript and review state remain in CrabDB, not VS Code global
  storage.

## Custom Providers

Add custom ACP commands in `crabdb.customProviders`. To keep CrabDB as the
source of truth, configure the command as a CrabDB relay entrypoint, for
example:

```json
{
  "crabdb.customProviders": [
    {
      "id": "custom-acp",
      "label": "Custom ACP via CrabDB",
      "command": "${crabdbPath}",
      "args": [
        "--workspace",
        "${workspaceFolder}",
...
provider is not recognized as CrabDB-backed because task history, checkpoints,
...
```sh
crabdb --workspace <temp-crabdb-workspace> acp doctor --agent claude-code --json
crabdb --workspace <temp-crabdb-workspace> acp doctor --agent fake --json
```
