...
mod tests {
    ...
    fn schema_and_ledger_recovery_errors_have_stable_contracts() {
        ...
        assert_eq!(
            schema.to_string(),
            "workspace schema version 17 cannot be opened; back up this workspace, then run `trail init --force --from-git` to create a fresh schema-v1 workspace from Git-tracked files"
        );
