...
pub(crate) fn handle_resource_read(db: &mut Trail, params: Value) -> Result<Value> {
...
fn resource_read_response(db: &mut Trail, args: ResourceReadArgs) -> Result<Value> {
...
fn templated_resource(db: &mut Trail, uri: &str) -> Result<(&'static str, String)> {
    ...
    if let Some(turn_id) =
        template_uri_argument(uri, "trail://workspace/turns/", "", RESOURCE_TURN_TEMPLATE)?
    {
        return Ok((
            "application/json",
    ...
    if let Some(run_id) =
        template_uri_argument(uri, "trail://workspace/runs/", "", RESOURCE_RUN_TEMPLATE)?
    {
        return Ok((
            "application/json",
    ...
    if let Some(span_id) =
        template_uri_argument(uri, "trail://workspace/spans/", "", RESOURCE_SPAN_TEMPLATE)?
    {
        return Ok((
            "application/json",
