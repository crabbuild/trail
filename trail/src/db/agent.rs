...
impl AgentAskRoute {
    ...
    fn tool(&self) -> &'static str {
        match self {
            AgentAskRoute::Inbox => "trail.agent_inbox",
            AgentAskRoute::Board => "trail.agent_board",
            AgentAskRoute::Stack => "trail.agent_stack",
            AgentAskRoute::Next => "trail.agent_next",
            AgentAskRoute::Guide => "trail.agent_guide",
            AgentAskRoute::Dashboard => "trail.agent_dashboard",
            AgentAskRoute::ReviewData => "trail.agent_review_data",
            AgentAskRoute::Actions => "trail.agent_review_data",
            AgentAskRoute::Summary => "trail.agent_summary",
            AgentAskRoute::Validate => "trail.agent_validate",
            AgentAskRoute::TestPlan => "trail.agent_test_plan",
            AgentAskRoute::Brief => "trail.agent_brief",
            AgentAskRoute::Story => "trail.agent_story",
            AgentAskRoute::Risk => "trail.agent_risk",
            AgentAskRoute::Impact => "trail.agent_impact",
            AgentAskRoute::ReviewMap => "trail.agent_review_map",
            AgentAskRoute::Confidence => "trail.agent_confidence",
            AgentAskRoute::Ready => "trail.agent_ready",
            AgentAskRoute::Diagnose => "trail.agent_diagnose",
            AgentAskRoute::Receipt => "trail.agent_receipt",
            AgentAskRoute::Handoff => "trail.agent_handoff",
            AgentAskRoute::Pr => "trail.agent_pr",
            AgentAskRoute::Changes => "trail.agent_changes",
            AgentAskRoute::ChangesByFile => "trail.agent_changes",
            AgentAskRoute::Tools => "trail.agent_tools",
            AgentAskRoute::TaskDiff { .. } => "trail.agent_diff",
            AgentAskRoute::TurnDiff { .. } => "trail.agent_diff",
            AgentAskRoute::Delta { .. } => "trail.agent_delta",
            AgentAskRoute::New { .. } => "trail.agent_new",
            AgentAskRoute::Files => "trail.agent_files",
            AgentAskRoute::File { .. } => "trail.agent_file",
            AgentAskRoute::Workdir => "trail.agent_workdir",
            AgentAskRoute::Checkpoints => "trail.agent_checkpoints",
            AgentAskRoute::Timeline => "trail.agent_timeline",
            AgentAskRoute::Turn => "trail.agent_turn",
            AgentAskRoute::Why(_) => "trail.agent_why",
            AgentAskRoute::ReviewFlow => "trail.agent_review_flow",
            AgentAskRoute::Review => "trail.agent_review",
            AgentAskRoute::Focus => "trail.agent_focus",
            AgentAskRoute::View => "trail.agent_view",
        }
    }
    ...
    fn cli_command(&self, selector: &str) -> String {
        ...
        match self {
            AgentAskRoute::Inbox => "trail agent inbox".to_string(),
            AgentAskRoute::Board => "trail agent board".to_string(),
            AgentAskRoute::Stack => "trail agent stack".to_string(),
            AgentAskRoute::Next => format!("trail agent next {selector}"),
            AgentAskRoute::Guide => format!("trail agent guide {selector}"),
            AgentAskRoute::Dashboard => format!("trail agent dashboard {selector}"),
            AgentAskRoute::ReviewData => format!("trail agent review-data {selector}"),
            AgentAskRoute::Actions => format!("trail agent action {selector}"),
            AgentAskRoute::Summary => format!("trail agent summary {selector}"),
            AgentAskRoute::Validate => format!("trail agent validate {selector}"),
            AgentAskRoute::TestPlan => format!("trail agent test-plan {selector}"),
            AgentAskRoute::Brief => format!("trail agent brief {selector}"),
            AgentAskRoute::Story => format!("trail agent story {selector}"),
            AgentAskRoute::Risk => format!("trail agent risk {selector}"),
            AgentAskRoute::Impact => format!("trail agent impact {selector}"),
            AgentAskRoute::ReviewMap => format!("trail agent review-map {selector}"),
            AgentAskRoute::Confidence => format!("trail agent confidence {selector}"),
            AgentAskRoute::Ready => format!("trail agent can-land {selector}"),
            AgentAskRoute::Diagnose => format!("trail agent recover {selector}"),
            AgentAskRoute::Receipt => format!("trail agent receipt {selector}"),
            AgentAskRoute::Handoff => format!("trail agent handoff {selector}"),
            AgentAskRoute::Pr => format!("trail agent pr {selector}"),
            AgentAskRoute::Changes => format!("trail agent changes {selector}"),
            AgentAskRoute::ChangesByFile => format!("trail agent changes {selector} --by-file"),
            AgentAskRoute::Tools => format!("trail agent tools {selector}"),
            AgentAskRoute::TaskDiff { file, patch } => {
                let mut command = format!("trail agent diff {selector}");
            ...
            AgentAskRoute::TurnDiff { file, patch } => {
                let mut command = format!("trail agent turn-diff {selector}");
                if let Some(path) = file {
                    command.push_str(&format!(" --file {}", agent_shell_arg(path)));
                }
                if *patch {
            ...
            AgentAskRoute::Delta { file, patch } => {
                let mut command = format!("trail agent last {selector}");
                if let Some(path) = file {
                    command.push_str(&format!(" --file {}", agent_shell_arg(path)));
                }
                if *patch {
            ...
            AgentAskRoute::New { file, patch } => {
                let mut command = format!("trail agent what-changed {selector}");
                if let Some(path) = file {
                    command.push_str(&format!(" --file {}", agent_shell_arg(path)));
                }
                if *patch {
            ...
            AgentAskRoute::Files => format!("trail agent changed-files {selector}"),
            AgentAskRoute::File { path, patch } => {
                let mut command =
                    format!("trail agent inspect {selector} {}", agent_shell_arg(path));
                if *patch {
                    command.push_str(" --patch");
                }
                command
            }
            AgentAskRoute::Workdir => format!("trail agent workdir {selector}"),
            AgentAskRoute::Checkpoints => format!("trail agent rewind-points {selector}"),
            AgentAskRoute::Timeline => format!("trail agent timeline {selector}"),
            AgentAskRoute::Turn => format!("trail agent turn {selector}"),
            AgentAskRoute::Why(path) => {
                format!("trail agent explain {selector} {}", agent_shell_arg(path))
            }
            AgentAskRoute::ReviewFlow => format!("trail agent review-flow {selector}"),
            AgentAskRoute::Review => format!("trail agent review-plan {selector}"),
            AgentAskRoute::Focus => format!("trail agent focus {selector}"),
            AgentAskRoute::View => format!("trail agent view {selector}"),
        }
    }
}
...
impl Trail {
    ...
    pub fn agent_stack_with_options(&self, include_archived: bool) -> Result<AgentStackReport> {
        ...
        if !include_archived {
            agent_push_suggestion(
                &mut suggestions,
                "trail agent stack --all".to_string(),
    ...
    pub fn agent_inbox_with_options(&self, include_archived: bool) -> Result<AgentInboxReport> {
        ...
        if archived_count == 0 {
            agent_push_suggestion(
                &mut suggestions,
                "trail agent inbox --all".to_string(),
        ...
        if let Some(latest) = tasks.first() {
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent brief {}", latest.lane),
    ...
    pub fn agent_guide(&self, selector: &str) -> Result<AgentGuideReport> {
        ...
        agent_push_suggestion(
            &mut suggestions,
            "trail agent ask what should I do next".to_string(),
    ...
    pub fn agent_ask(&mut self, selector: &str, question: &str) -> Result<AgentAskReport> {
        ...
        let suggestions = vec![
            StatusSuggestion {
                ...
                reason: "run the routed Trail view from the CLI".to_string(),
            },
            StatusSuggestion {
                command: format!(
                    "trail agent ask --selector {} {}",
    ...
    pub fn agent_workdir(&self, selector: &str) -> Result<AgentWorkdirReport> {
        ...
        suggestions.push(StatusSuggestion {
            command: format!("trail agent brief {lane}"),
    ...
    pub fn agent_story(&self, selector: &str) -> Result<AgentStoryReport> {
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent changes {}", view.task.lane),
        ...
        if view.task.workdir.is_some() {
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent workdir {}", view.task.lane),
    ...
    pub fn agent_receipt(&self, selector: &str) -> Result<AgentReceiptReport> {
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent ready {}", report.task.lane),
            "check whether this task can be safely applied to Git",
        );
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent changes {}", report.task.lane),
            "inspect high-level change cards behind this receipt",
        );
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent pr {}", report.task.lane),
            "draft a pull request title and body from this task",
        );
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent review-plan {}", report.task.lane),
    ...
    pub fn agent_pr_draft(&self, selector: &str) -> Result<AgentPrDraftReport> {
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent ready {}", receipt.task.lane),
            "confirm apply readiness before opening or updating a PR",
        );
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent land {} --dry-run", receipt.task.lane),
            "preview the Git commit and fast-forward plan",
        );
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent receipt {}", receipt.task.lane),
    ...
    pub fn agent_validate(&self, selector: &str) -> Result<AgentValidationReport> {
        ...
        let next = suggestions
            ...
            .cloned()
            .unwrap_or_else(|| StatusSuggestion {
                command: format!("trail agent ready {lane}"),
    ...
    pub fn agent_focus(
        ...
        let next = if patches {
            ...
            StatusSuggestion {
                command: format!(
                    "trail agent focus {lane} --file {} --patch",
    ...
    pub fn agent_checkpoints(&self, selector: &str) -> Result<AgentCheckpointReport> {
        ...
        if !entries.is_empty() {
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent rewind {lane} --to before-last-turn"),
                "rewind to the state before the most recent completed turn",
            );
        }
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent changes {lane}"),
    ...
    pub fn agent_compare(
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent review-plan {}", left_view.task.lane),
            "inspect the left task readiness, transcript, and changed paths",
        );
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent review-plan {}", right_view.task.lane),
            "inspect the right task readiness, transcript, and changed paths",
        );
        if let Some(shared) = shared_paths.first() {
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent why {} {}", left_view.task.lane, shared.path),
                "explain the first shared file in the left task",
            );
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent why {} {}", right_view.task.lane, shared.path),
                "explain the first shared file in the right task",
            );
        } else {
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent land {} --dry-run", left_view.task.lane),
                "preview applying the left task",
            );
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent land {} --dry-run", right_view.task.lane),
    ...
    pub fn agent_new(
        ...
        let diff = Some(AgentDiffReport {
            ...
            suggestions: vec![
                StatusSuggestion {
                    command: format!("trail agent changes {lane}"),
    ...
    pub fn agent_change_set(
        ...
        let next = if patches {
            StatusSuggestion {
                command: format!("trail agent review-plan {}", changes.lane),
                reason: "return to the task review dashboard after inspecting this change set"
                    .to_string(),
            }
        } else {
            StatusSuggestion {
                command: format!("trail agent change {} {} --patch", changes.lane, card.key),
    ...
    pub fn agent_why(&self, selector: &str, path: &str) -> Result<AgentWhyReport> {
                ...
                _ => {
                    if let Some(operation) = &group.operation_id {
                        agent_push_suggestion(
                            &mut suggestions,
                            format!(
                                "trail agent diff {lane} --operation {} --file {} --patch",
                                operation.0,
                                agent_shell_arg(&path)
                            ),
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent review-plan {lane}"),
    ...
    pub fn agent_diff(
        ...
        if let Some(path) = &file_filter {
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent why {lane} {}", agent_shell_arg(path)),
    ...
    fn agent_inbox_next_for_task(&self, task: &AgentTaskReport) -> Result<StatusSuggestion> {
        if task.archived {
            return Ok(StatusSuggestion {
                command: format!("trail agent unarchive {}", task.lane),
    ...
    fn normalize_agent_query_path(&self, view: &AgentTaskViewReport, path: &str) -> Result<String> {
        ...
        let materialized_prefix = format!(".trail/worktrees/{}/", view.task.lane);
...
fn agent_turn_by_user_selector<'a>(
    ...
    if let Ok(index) = selector.parse::<usize>() {
        if index == 0 {
            return Err(Error::InvalidInput(
                "turn index is 1-based; use `trail agent turn 1` for the first turn".to_string(),
            ));
        }
        return transcript
            .turns
...
fn agent_board_column_specs() -> Vec<AgentBoardColumnSpec> {
    vec![
        ...
        AgentBoardColumnSpec {
            key: "applied",
            label: "Applied",
            summary: "Tasks have already landed in Git or Trail main.",
            attention: false,
        },
        AgentBoardColumnSpec {
            key: "archived",
...
fn agent_stack_item_next(task: &AgentTaskReport, status: &str) -> StatusSuggestion {
    ...
    match status {
        ...
        "archived" => StatusSuggestion {
            command: format!("trail agent receipt {lane}"),
            reason: "inspect the archived task receipt if needed".to_string(),
        },
        _ => StatusSuggestion {
            command: "trail agent inbox".to_string(),
...
fn agent_guide_headline(task: Option<&AgentTaskReport>, status: &AgentTaskStatus) -> String {
    match task {
        ...
        None => match status {
            AgentTaskStatus::Empty => {
                "Set up one editor or terminal entrypoint; Trail will create fresh tasks automatically."
                    .to_string()
            }
            _ => "Use Trail agent commands to inspect, validate, apply, or recover agent work."
                .to_string(),
        },
    }
}
...
fn agent_guide_steps(
    ...
    agent_push_guide_step(
        &mut steps,
        "Show actions",
        format!("trail agent action {lane}"),
        "list the safe review, validation, apply, and recovery actions Trail can run for this task",
    ...
    match task.status {
        AgentTaskStatus::Active => {
            steps.push(agent_guide_step(
                "Watch the task",
                format!("trail agent dashboard {lane}"),
        ...
        AgentTaskStatus::Ready => {
            ...
            steps.push(agent_guide_step(
                "Validate",
                format!("trail agent validate {lane}"),
        ...
        AgentTaskStatus::Blocked | AgentTaskStatus::Conflicted => {
            steps.push(agent_guide_step(
                "Diagnose",
                format!("trail agent recover {lane}"),
            ...
            steps.push(agent_guide_step(
                ...
                format!("trail agent rewind-points {lane}"),
        ...
        AgentTaskStatus::Applied => {
            ...
            steps.push(agent_guide_step(
                "Start follow-up",
                format!("trail agent continue {lane} --provider {provider}"),
    ...
    agent_push_guide_step(
        ...
        "Ask naturally",
        format!(
            "trail agent ask --selector {lane} {}",
            agent_shell_arg("what should I do next")
        ),
        "route a plain-language question to the right Trail view",
    ...
    agent_push_guide_step(
        &mut steps,
        "See new work",
        format!("trail agent what-changed {lane}"),
...
fn agent_guide_concepts() -> Vec<AgentGuideConcept> {
    vec![
        AgentGuideConcept {
            ...
            meaning:
                "one agent job with its own isolated Trail lane, transcript, tools, and checkpoints"
                    .to_string(),
        },
        AgentGuideConcept {
        ...
        for event in &turn.events {
            ...
            entry
                ...
                .or_insert_with(|| AgentToolTurnRef {
                    ...
                    turn_command: format!("trail agent turn {lane} {index}"),
                    diff_command: group
...
fn agent_tools_suggestions(lane: &str, tools: &[AgentToolEntry]) -> Vec<StatusSuggestion> {
    let mut suggestions = vec![
        ...
        StatusSuggestion {
            command: format!("trail agent timeline {lane}"),
...
fn agent_impact_areas(lane: &str, changed_paths: &[FileDiffSummary]) -> Vec<AgentImpactArea> {
    ...
    for change in changed_paths {
        ...
        let area = areas
            ...
            .or_insert_with(|| AgentImpactArea {
                ...
                changed_lines: 0,
                reasons: Vec::new(),
                review_command: format!("trail agent changes {lane} --by-file"),
                diff_command: None,
            });
    ...
    for area in &mut areas {
        ...
        if area.changed_paths.len() == 1 {
            ...
            area.review_command = format!("trail agent file {lane} {}", agent_shell_arg(path));
            area.diff_command = Some(format!(
                "trail agent file {lane} {} --patch",
                agent_shell_arg(path)
            ));
...
fn agent_impact_suggestions(
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent review-flow {lane}"),
                        ...
                        .map(|priority| {
                            ...
                            AgentReviewMapFile {
                                ...
                                review_command: format!(
                                    "trail agent focus {lane} --file {file_arg}"
                                ),
...
fn agent_review_map_next(
    ...
    if progress.status != "up_to_date" {
        return StatusSuggestion {
            command: format!("trail agent new {lane}"),
    ...
    StatusSuggestion {
        command: format!("trail agent confidence {lane}"),
...
fn agent_review_map_suggestions(
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent impact {lane}"),
...
fn agent_risk_report_from_view(view: &AgentTaskViewReport) -> AgentRiskReport {
    ...
    if view.task.status == AgentTaskStatus::Applied {
        return AgentRiskReport {
            ...
            score: 0,
            summary:
                "Task has already been applied; use `trail agent continue` for follow-up work."
                    .to_string(),
            reasons,
        ...
        })
    {
        agent_push_suggestion(
            &mut recommendations,
            format!("trail agent workdir {lane}"),
    ...
    if let Some(error) = apply_error {
        if error.contains("current Git worktree has tracked changes") {
            return StatusSuggestion {
                ...
                reason: "inspect tracked Git changes before running Trail apply".to_string(),
            };
        }
        if error.contains("does not match Trail branch") {
            return StatusSuggestion {
                command: format!("trail git import-update --branch {crab_branch}"),
                reason: "refresh Trail's internal base or switch to a matching Git branch"
                    .to_string(),
            };
        }
    }
...
fn agent_confidence_factors(
    ...
    let (validation_state, validation_delta, validation_message, validation_command) =
        if validation.needs_test && validation.needs_eval {
            ...
            (
                ...
                Some(format!("trail agent validate {lane}")),
            )
        };
    factors.push(agent_confidence_factor(
        "validation",
    ...
    let (apply_state, apply_delta, apply_message, apply_command) = if ready.ready {
        (
            "pass",
            0,
            "Git dry-run preflight and Trail readiness passed".to_string(),
            Some(format!("trail agent finish {lane} --dry-run")),
        )
    } else {
        match ready.status.as_str() {
            "applied" => (
                ...
                0,
                "task has already been applied".to_string(),
                Some(format!("trail agent finish {lane}")),
            ),
            "needs_record" => (
                "warn",
                -20,
...
fn agent_confidence_suggestions(
    ...
    if verdict == "go" {
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent finish {lane} --dry-run"),
...
fn agent_review_data_actions(
    ...
    if let Some(focus) = focus {
        ...
        let open_command = focus
            .open_command
            .clone()
            .unwrap_or_else(|| format!("trail agent open {lane} --file {path_arg}"));
        ...
        actions.push(agent_review_action(
            ...
            "Inspect focus file",
            "inspect_file",
            format!("trail agent focus {lane} --file {path_arg}"),
        ...
        actions.push(agent_review_action(
            ...
            "Show focus patch",
            "show_patch",
            format!("trail agent focus {lane} --file {path_arg} --patch"),
        ...
        actions.push(agent_review_action(
            ...
            "Mark file reviewed",
            "mark_file_reviewed",
            format!("trail agent done-file {lane} {path_arg}"),
            ...
            Some(focus.path.clone()),
            focus.open_path.clone(),
            Some("trail.agent_mark_file_reviewed"),
            Some(serde_json::json!({
                "selector": lane,
                "path": focus.path,
                "note": null
    ...
    actions.push(agent_review_action(
        ...
        "Show review map",
        "show_review_map",
        format!("trail agent review-map {lane}"),
    ...
    actions.push(agent_review_action(
        ...
        "Show test plan",
        "show_test_plan",
        format!("trail agent test-plan {lane}"),
    ...
    actions.push(agent_review_action(
        ...
        "Preview apply",
        "apply_dry_run",
        format!("trail agent apply {lane} --dry-run"),
    ...
    if needs_review_files == 0 && confidence.review_status != "up_to_date" {
        actions.push(agent_review_action(
            ...
            None,
            None,
            Some("trail.agent_mark_reviewed"),
            Some(serde_json::json!({
                "selector": lane,
                "note": null
            })),
...
fn agent_review_action_mcp_arguments(
    ...
    match tool {
        Some("trail.agent_ready") => Some(serde_json::json!({ "selector": lane })),
        Some("trail.agent_test_plan") => Some(serde_json::json!({ "selector": lane })),
        Some("trail.agent_test") | Some("trail.agent_eval") => {
...
fn agent_review_data_suggestions(
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent can-land {lane}"),
...
fn agent_already_applied_suggestions(task: &AgentTaskReport) -> Vec<StatusSuggestion> {
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent view {}", task.lane),
        "inspect the applied task transcript, tools, and checkpoint",
    );
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent finish {}", task.lane),
        "hide the applied task from the default inbox when you are done",
    );
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent continue {} --provider {provider}", task.lane),
...
fn agent_finish_suggestions(
    ...
    if status == "finished" {
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent continue {lane}"),
...
fn agent_validation_suggestions(
    ...
    if needs_eval {
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent eval {lane} -- <eval-command>"),
...
fn agent_test_plan_steps(
    ...
    if validation.needs_test {
        ...
        steps.push(AgentTestPlanStep {
            ...
            required: false,
            command: format!("trail agent validate {lane}"),
    ...
    if validation.needs_eval {
        steps.push(AgentTestPlanStep {
            ...
            command: format!("trail agent eval {lane} -- <eval-command>"),
...
fn agent_diagnosis_recovery_options(
    ...
    if !checkpoints.entries.is_empty() {
        ...
        agent_push_suggestion(
            &mut options,
            format!("trail agent checkpoints {lane}"),
    ...
    agent_push_suggestion(
        &mut options,
        format!("trail agent summary {lane}"),
...
fn agent_diagnosis_suggestions(
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent focus {lane} --patch"),
...
fn agent_focus_suggestions(
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent why {lane} {file_arg}"),
...
fn agent_turn_suggestions(
    ...
    if !patches {
        let command = if let Some(file) = file {
            format!(
                "trail agent turn {lane} {index} --file {} --patch",
                agent_shell_arg(file)
            )
        } else {
            format!("trail agent turn {lane} {index} --patch")
        };
        agent_push_suggestion(
            &mut suggestions,
            command,
...
fn agent_change_cards(
    ...
    for (key, (title, mut changed_paths)) in buckets {
        ...
        let focus_command = first_path
            .map(|path| format!("trail agent focus {lane} --file {}", agent_shell_arg(path)));
        let why_command =
            first_path.map(|path| format!("trail agent why {lane} {}", agent_shell_arg(path)));
        let diff_command = first_path.map(|path| {
            if changed_paths.len() == 1 {
                touched_by
                    ...
                    .unwrap_or_else(|| {
                        format!(
                            "trail agent diff {lane} --file {} --patch",
                            agent_shell_arg(path)
                        )
                    })
                ...
                touch
                    .diff_command
                    .clone()
                    .unwrap_or_else(|| format!("trail agent diff {lane} --patch"))
            } else {
                format!("trail agent diff {lane} --patch")
            }
        });
    ...
    ranked
        ...
        .map(|(index, (_, _, _, mut card))| {
            card.rank = index + 1;
            card.review_command = format!("trail agent change {lane} {}", card.rank);
            card
        })
        .collect()
}
...
fn agent_file_change_cards(
    ...
    for change in changed_paths {
        ...
        let diff_command = touched_by
            .first()
            .map(|touch| agent_file_diff_command(lane, touch, &path))
            .unwrap_or_else(|| format!("trail agent diff {lane} --file {file_arg} --patch"));
        ...
        ranked.push((
            ...
            AgentChangeCard {
                ...
                tool_summaries,
                review_command: String::new(),
                focus_command: Some(format!("trail agent focus {lane} --file {file_arg}")),
                why_command: Some(format!("trail agent why {lane} {file_arg}")),
...
fn agent_changes_next(lane: &str, cards: &[AgentChangeCard]) -> StatusSuggestion {
    ...
    StatusSuggestion {
        command: format!("trail agent next {lane}"),
        reason: "ask Trail for the next useful action".to_string(),
    }
}
...
fn agent_change_set_suggestions(
    ...
    if patches {
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent change {lane} {}", card.key),
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent timeline {lane}"),
...
fn agent_file_next(
    ...
    if !matched {
        return StatusSuggestion {
            command: format!("trail agent files {lane}"),
            reason: "list files this agent task actually changed".to_string(),
        };
    }
    if !patches {
        return StatusSuggestion {
            command: format!("trail agent file {lane} {} --patch", agent_shell_arg(path)),
            reason: "show the focused patch for this file".to_string(),
        };
    }
    if let Some(card) = first_card {
        return StatusSuggestion {
            command: format!("trail agent change {lane} {}", card.key),
            reason: "review the full change set containing this file".to_string(),
        };
    }
    StatusSuggestion {
        command: format!("trail agent why {lane} {}", agent_shell_arg(path)),
    ...
    if matched {
        ...
        if let Some(card) = cards.first() {
            ...
            if !patches {
                agent_push_suggestion(
                    &mut suggestions,
                    format!("trail agent change {lane} {} --patch", card.key),
...
fn agent_delta_next(
    ...
    if !patches {
        ...
        let file_arg = file_filter
            ...
            .unwrap_or_default();
        return StatusSuggestion {
            command: format!("trail agent delta {lane}{mode_flag}{file_arg} --patch"),
    ...
    if let Some(group) = group {
        match group.kind.as_str() {
            "turn" => {
                agent_push_suggestion(
                    &mut suggestions,
                    format!("trail agent turn {lane} {}", group.index),
            ...
            _ => {
                if let Some(operation) = &group.operation_id {
                    agent_push_suggestion(
                        &mut suggestions,
                        format!(
                            "trail agent diff {lane} --operation {} --patch",
                            operation.0
                        ),
        ...
        if let Some(path) = file_filter {
            let command = if matched && !patches {
                format!(
                    "trail agent delta {lane}{} --file {} --patch",
                    if mode == "operation" {
                        " --by-operation"
                    } else {
                        ""
                ...
                format!("trail agent file {lane} {} --patch", agent_shell_arg(path))
            };
            agent_push_suggestion(
                &mut suggestions,
                command,
    ...
    match status {
        ...
        "new_changes" | "unreviewed" if !patches => {
            let file_arg = file_filter
                ...
                .unwrap_or_default();
            StatusSuggestion {
                command: format!("trail agent new {lane}{file_arg} --patch"),
                reason: "show the exact patch for changes not yet marked reviewed".to_string(),
            }
        }
        _ => StatusSuggestion {
            command: format!("trail agent done {lane}"),
...
fn agent_new_suggestions(
    ...
    if matches!(status, "new_changes" | "unreviewed") {
        if !patches {
            ...
            agent_push_suggestion(
                &mut suggestions,
                format!("trail agent new {lane}{file_arg} --patch"),
    ...
    agent_push_suggestion(
        &mut suggestions,
        format!("trail agent delta {lane} --patch"),
...
fn agent_mark_file_reviewed_suggestions(lane: &str, path: &str) -> Vec<StatusSuggestion> {
    vec![
        StatusSuggestion {
            command: format!("trail agent review-map {lane}"),
...
fn agent_archive_suggestions(task: &AgentTaskReport, archived: bool) -> Vec<StatusSuggestion> {
    ...
    if archived {
        ...
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent unarchive {lane}"),
...
fn agent_timeline_view_command(lane: &str, group: &AgentChangeGroup) -> Option<String> {
    match group.kind.as_str() {
        "turn" => Some(format!("trail agent turn {lane} {}", group.index)),
        _ => group
            .operation_id
            .as_ref()
            .map(|operation| format!("trail agent diff {lane} --operation {}", operation.0)),
    }
}
...
fn agent_timeline_rewind_before_command(lane: &str, group: &AgentChangeGroup) -> Option<String> {
    ...
    match group.kind.as_str() {
        "turn" => Some(format!(
            "trail agent rewind {lane} --to before-turn:{}",
            group.index
        )),
        _ => group
            .before_change
            .as_ref()
            .map(|change| format!("trail agent rewind {lane} --to {}", change.0)),
    }
}
...
fn agent_timeline_suggestions(
    ...
    if mode != "operation" {
        agent_push_suggestion(
            &mut suggestions,
            format!("trail agent timeline {lane} --by-operation"),
            "zoom in from prompt turns to recorded Trail operations",
        );
    }
            ...
            let diff_command = touched_by
                ...
                .map(|touch| match touch.kind.as_str() {
                    ...
                    _ => touch
                        ...
                        .map(|operation| {
                            format!(
                                "trail agent diff {lane} --operation {} --file {file_arg} --patch",
                                operation.0
                            )
                        })
                        .unwrap_or_else(|| {
                            format!("trail agent diff {lane} --file {file_arg} --patch")
                        }),
                })
                .or_else(|| Some(format!("trail agent diff {lane} --file {file_arg} --patch")));
            AgentReviewPriority {
                rank: 0,
                change: change.clone(),
                score,
                reasons,
                touched_by,
                why_command: format!("trail agent why {lane} {file_arg}"),
                diff_command,
            }
        })
        .collect::<Vec<_>>();
...
fn agent_report_markdown(
    ...
    if !changes.groups.is_empty() {
        ...
        out.push_str(&format!("trail agent why {} <path>\n", view.task.lane));
        out.push_str("```\n");
    }
    out
}
...
fn agent_receipt_markdown(
    ...
    out.push_str(&format!("```sh\ntrail agent ready {}\n", report.task.lane));
    out.push_str(&format!("trail agent changes {}\n", report.task.lane));
    out.push_str(&format!("trail agent review-plan {}\n", report.task.lane));
    out.push_str(&format!(
        "trail agent land {} --dry-run\n",
        report.task.lane
    ));
    out.push_str("```\n");
    out
...
fn agent_handoff_markdown(
    ...
    out.push_str("Use this packet to continue, review, validate, or apply the recorded agent task without reading Trail internals.\n\n");
    ...
    out.push_str(&format!("trail agent changes {}\n", report.task.lane));
    out.push_str(&format!("trail agent review-plan {}\n", report.task.lane));
    out.push_str(&format!("trail agent ready {}\n", report.task.lane));
    out.push_str(&format!(
        "trail agent land {} --dry-run\n",
        report.task.lane
    ));
...
fn agent_checkpoint_entry(lane: &str, group: &AgentChangeGroup) -> AgentCheckpointEntry {
    ...
    let rewind_before_command = before_target
        .as_ref()
        .map(|target| format!("trail agent rewind {lane} --to {target}"));
...
fn agent_file_entry(
    ...
    AgentFileEntry {
        change: change.clone(),
        touched_by,
        why_command: format!("trail agent why {lane} {}", agent_shell_arg(&change.path)),
        diff_command,
        report_command: format!("trail agent report {lane} --markdown"),
    }
}
...
fn agent_compare_recommendation(
    ...
    if let Some(shared) = shared_paths.first() {
        return StatusSuggestion {
            command: format!("trail agent why {} {}", left.lane, shared.path),
    ...
    if matches!(
        ...
        return StatusSuggestion {
            command: format!("trail agent review-plan {}", left.lane),
    ...
    if matches!(
        ...
        return StatusSuggestion {
            command: format!("trail agent review-plan {}", right.lane),
    ...
    StatusSuggestion {
        command: format!("trail agent land {} --dry-run", preferred.lane),
...
fn agent_turn_diff_command(
    ...
    let mut command = format!("trail agent turn-diff {lane}");
