...
impl Trail {
        ...
        Ok(Some(ConflictExplanation {
            ...
            next_steps: vec![
                "Inspect each conflicted path and choose source, target, or manual content."
                    .to_string(),
                format!(
                    "Resolve with `trail conflicts resolve {} --take source`, `--take target`, or `--manual <JSON_FILE>`.",
    ...
    fn explain_conflict_path(
        ...
        let reason = if !lines.is_empty() {
            ...
            "both sides changed the same path and Trail could not prove a safe line merge"
                .to_string()
        };
