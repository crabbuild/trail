...
>
> **Drift check (run first)**: `git diff --stat 6bb6fa7..HEAD -- crates/crabdb/src/db/storage/content.rs crates/crabdb/src/db/storage/manifest.rs crates/crabdb/src/db/lane/lifecycle.rs crates/crabdb/src/db/record/checkout.rs`
>
...
- `crates/crabdb/src/db/storage/content.rs` exposes `load_root_files` around lines 124-139, returning a full `BTreeMap`.
...
- `crates/crabdb/src/db/lane/lifecycle.rs` uses full-root loading for non-sparse lane materialization.
- `crates/crabdb/src/db/record/checkout.rs` loads current root files and target root files around lines 38-48 before diffing them.
- `crates/crabdb/src/db/storage/manifest.rs` already has streaming sorted-merge patterns in `diff_root_to_worktree_index` and `diff_root_to_disk_manifest`.
...
| Focused tests | `cargo test -p crabdb streaming_root_materialization` | all new tests pass |
| Checkout regression | `cargo test -p crabdb checkout` | all matching checkout tests pass |
...
- `crates/crabdb/src/db/storage/manifest.rs` only as an iterator exemplar or for shared helper extraction
...
**Verify**: `cargo test -p crabdb streaming_root_materialization_chunks` -> new chunk tests pass after they are added.
...
**Verify**: `cargo test -p crabdb streaming_root_materialization` -> all new tests pass.
...
- Existing checkout regression via `cargo test -p crabdb checkout`.
